...
func printUsage() {
	...
	fmt.Println("  kitten compose <config.json>")
...
func composeCommand() {
	...
	if len(os.Args) < 3 {
		...
		fmt.Println("Usage: kitten compose <config.json>")
		os.Exit(1)
	}
	...
	manager, err := kitten.NewManagerFromFile(configPath)
	if err != nil {
	...
	fmt.Println("\nAll containers are running. Press Ctrl+C to stop.")

	go func() {
		<-sigChan
		fmt.Println("\nReceived signal, stopping containers...")
	...
	fmt.Println("All containers exited")
}
